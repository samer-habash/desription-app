resource "aws_security_group" "description_sg" {
  name = var.sg_name
...
resource "aws_security_group_rule" "inbound_allow_app_port" {
  ...
  security_group_id = aws_security_group.description_sg.id
  cidr_blocks = ["0.0.0.0/0"]
}

resource "aws_security_group_rule" "inbound_allow_ssh_port" {
  description = "Allowing Port 5000 from all over the world"
  type = "ingress"
